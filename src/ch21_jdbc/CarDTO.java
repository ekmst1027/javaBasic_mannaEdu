package ch21_jdbc;

// 모델(model) : 데이터를 저장하는 그릇 역할
// DTO : Data Transfer Object - 데이터 전달 객체
// DAO : Data Access Object - 데이터 접근 객체

// CarDTO.java - car 테이블의 레코드 1개를 저장하는 클래스
// CarDAO.java - car 테이블에 데이터 처리를 하는 클래스
// CarManage.java - car 테이블을 사용하는 클래스
public class CarDTO {
	// 텡블의 필드명과 자료형 그대로 자바 변수로 선언
	private String license_number;
	private String company;
	private String type;
	private int year;
	private int efficiency;
	
	// Constructor
	public CarDTO() {
		// TODO Auto-generated constructor stub
	}
	
	public CarDTO(String license_number, String company, String type, int year, int efficiency) {
		this.license_number = license_number;
		this.company = company;
		this.type = type;
		this.year = year;
		this.efficiency = efficiency;
	}




	// Getters and Setters
	public String getLicense_number() {
		return license_number;
	}
	public void setLicense_number(String license_number) {
		this.license_number = license_number;
	}
	public String getCompany() {
		return company;
	}
	public void setCompany(String company) {
		this.company = company;
	}
	public String getType() {
		return type;
	}
	public void setType(String type) {
		this.type = type;
	}
	public int getYear() {
		return year;
	}
	public void setYear(int year) {
		this.year = year;
	}
	public int getEfficiency() {
		return efficiency;
	}
	public void setEfficiency(int efficiency) {
		this.efficiency = efficiency;
	}
	
	// ToString()
	@Override
	public String toString() {
		return "CarDTO [license_number=" + license_number + ", company=" + company + ", type=" + type + ", year=" + year
				+ ", efficiency=" + efficiency + "]";
	}
	
}
